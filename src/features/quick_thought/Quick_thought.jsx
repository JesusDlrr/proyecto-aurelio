...
import { FaBuffer, FaFileImage } from "react-icons/fa";
...
export const Quick_Thought = ({ makePost }) => {
    ...
    return (
        <>
            <div className="flex w-full h-full flex-row items-center gap-2 rounded-md border border-gray-900/10 bg-quick4 p-2">
                <div className="flex">
                    <button
                        className="relative h-10 max-h-[40px] w-10 max-w-[40px] select-none text-center align-middle font-sans text-xs font-medium uppercase text-gray-900 transition-all hover:bg-gray-900/10 active:bg-gray-900/20 disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"
                        type="button">
                ...
                <div className="relative grid h-full w-full min-w-[200px]">
                    <textarea rows="1" placeholder="Your Message" value={post_message} onChange={(e) => { handleSetPostMessage(e.target.value) }}
                        className="peer h-full  min-h-full w-full resize-none rounded-[7px]  !border-0 border-blue-gray-200 border-t-transparent bg-transparent px-3 py-2.5 font-sans text-sm font-normal text-white outline outline-0 transition-all placeholder:text-blue-gray-300 placeholder-shown:border placeholder-shown:border-blue-gray-200 placeholder-shown:border-t-blue-gray-200 focus:border-2 focus:border-gray-900 focus:border-transparent focus:border-t-transparent focus:outline-0 disabled:resize-none disabled:border-0 disabled:bg-blue-gray-50"></textarea>
                    <label
                ...
                <div>
                    <button
                        ...
                        className="relative h-10 max-h-[40px] w-10 max-w-[40px] select-none rounded-full text-center align-middle font-sans text-xs font-medium uppercase text-white transition-all hover:bg-gray-900/10 active:bg-gray-900/20 disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"
                        type="button">
                        <span className="absolute transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
                            <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path
                ...
                <span className='text-white'>{`${post_message.length}/${message_max_chars}`}</span>
            </div>
            {/* </div> */}
