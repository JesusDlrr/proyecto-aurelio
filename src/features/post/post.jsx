    ...
    <>
      <div className="bg-gray-50 dark:bg-quick4 dark:text-white sm:flex border-b border-quick5">
        <div className="sm:mt-0 sm:text-left break-all w-full">
          ...
          <div
            className="cursor-pointer hover:bg-slate-200 dark:hover:bg-quick5 hover:rounded p-3"
            onClick={() => {
                ...
                <button>
                  <IoIosFlag
                    onClick={() => {
                      axios
                        .post(
                          `https://quick-api-9c95.onrender.com/administration/flag/post/${post.id}`,
                          {},
                          {
          ...
          <div className="p-3">
            <p className="text-gray-600 font-serif dark:text-white lg:pb-8 content-start">
              {is_repost ? post.original_post.content : post.content}
            </p>
            {!is_repost ? (
              <div
                className={post.media.length > 1 && "grid grid-cols-2 gap-1"}
              >
                {post.media.length > 1
                  ? post.media
                      .slice(0, 4)
            ...
            <div className="flex gap-x-5 pt-7">
              <div className="flex items-center space-x-4">
                ...
                <h1 className="text-md text-gray-500 dark:text-gray-400">
                  {is_repost ? post.original_post.likes : post.likes}
                </h1>
              </div>
              <div className="flex items-center space-x-4">
                {
                  ...
                  >
                    {updating_reposts ? (
                      ...
                      <svg
                        className={`cursor-pointer h-6 w-6 ml-2 items-center ${
                          post.reposted ? "fill-green-400" : "dark:fill-white"
                        }`}
                        viewBox="0 0 24 24"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                ...
                <h1 className="text-md text-gray-500 dark:text-gray-400">
                  {is_repost ? post.original_post.reposts : post.reposts}
                </h1>
              </div>
            </div>
          </div>
