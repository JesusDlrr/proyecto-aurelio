...
const useRepost = (post_id) => {
  ...
  const repost = () => {
    if (repostable) {
      ...
      if (reposted) {
        axios
          .delete(`https://quick-api-9c95.onrender.com/repost/${post_id}`, {
        ...
        axios
          .post(
            `https://quick-api-9c95.onrender.com/repost/${post_id}`,
            {},
            {
              params: { user_id: user.uid },
            }
  ...
  const likePost = () => {
    if (likeable) {
      ...
      if (liked) {
        axios
          .delete(`https://quick-api-9c95.onrender.com/likes/${post_id}`, {
            params: { user_id: user.uid },
          })
          .then((response) => {
            if (response.status === 200) {
        ...
        axios
          .post(
            `https://quick-api-9c95.onrender.com/likes/${post_id}`,
            {},
            {
              params: { user_id: user.uid },
            }
